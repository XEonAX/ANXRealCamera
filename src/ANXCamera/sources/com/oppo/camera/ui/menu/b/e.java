...
public class e {
    ...
    private float[][] f2741a = ((float[][]) Array.newInstance(float.class, new int[]{10, 16}));

    /* renamed from: b  reason: collision with root package name */
    private float[] f2742b = new float[16];
    ...
    public void b() {
        this.h++;
        for (int i = 0; i < 16; i++) {
            this.f2741a[this.h][i] = this.e[i];
        }
    }

    public void c() {
        for (int i = 0; i < 16; i++) {
            this.e[i] = this.f2741a[this.h][i];
        }
        this.h--;
    }

    public float[] d() {
        Matrix.multiplyMM(this.f2742b, 0, this.d, 0, this.e, 0);
