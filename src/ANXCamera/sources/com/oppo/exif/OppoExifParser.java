...
class OppoExifParser {
     ...
     */
    private boolean seekTiffData(InputStream inputStream) throws IOException, OppoExifInvalidFormatException {
        OppoCountedDataInputStream oppoCountedDataInputStream = new OppoCountedDataInputStream(inputStream);
        if (oppoCountedDataInputStream.readShort() == -40) {
            short readShort = oppoCountedDataInputStream.readShort();
            while (true) {
                if (readShort == -39 || OppoJpegHeader.isSofMarker(readShort)) {
                    break;
                }
                int readUnsignedShort = oppoCountedDataInputStream.readUnsignedShort();
                if (readShort == -31 && readUnsignedShort >= 8) {
                    int readInt = oppoCountedDataInputStream.readInt();
                    short readShort2 = oppoCountedDataInputStream.readShort();
                    readUnsignedShort -= 6;
                    if (readInt == EXIF_HEADER && readShort2 == 0) {
                        this.mTiffStartPosition = oppoCountedDataInputStream.getReadByteCount();
                ...
                if (j != oppoCountedDataInputStream.skip(j)) {
                    break;
                }
                readShort = oppoCountedDataInputStream.readShort();
            }
            return false;
        }
