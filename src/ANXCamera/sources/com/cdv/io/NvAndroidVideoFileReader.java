...
public class NvAndroidVideoFileReader implements SurfaceTexture.OnFrameAvailableListener {
    ...
    public boolean OpenFile(String str, int i, AssetManager assetManager, int i2, long j) {
        String str2 = str;
        int i3 = i;
        ...
        try {
            ...
            if (assetManager2 == null) {
                ...
                AssetFileDescriptor openFd = assetManager2.openFd(str);
            ...
            while (true) {
                if (i4 >= trackCount) {
                    break;
                } else if (this.m_extractor.getTrackFormat(i4).getString("mime").startsWith("video/")) {
                    this.m_videoTrackIndex = i4;
                    break;
                } else {
                    i4++;
                }
            }
            int i5 = this.m_videoTrackIndex;
            if (i5 < 0) {
                Log.e(TAG, "Failed to find a video track from " + str);
                CloseFile();
                return false;
            }
            this.m_extractor.selectTrack(i5);
