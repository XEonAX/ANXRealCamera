...
public class NvsMultiThumbnailSequenceView extends HorizontalScrollView {
    ...
    private static final int THUMBNAIL_SEQUENCE_FLAGS_AUTO_CHECK_DECODEC_MODE = 1;
    private static final int THUMBNAIL_SEQUENCE_FLAGS_EXACT_FRAME = 2;
    private static final int THUMBNAIL_SEQUENCE_FLAGS_ONLY_DECODEC_KEY_FRAME = 4;
    ...
    public static class ThumbnailSequenceDesc {
        ...
        public boolean onlyDecodecKeyFrame = false;
    ...
    private boolean isOnlyDecodecKeyFrame(String str, long j) {
        NvsStreamingContext instance = NvsStreamingContext.getInstance();
        if (instance == null) {
            return false;
        }
        NvsAVFileInfo aVFileInfo = instance.getAVFileInfo(str);
        if (aVFileInfo == null || aVFileInfo.getVideoStreamCount() < 1) {
            return false;
        }
        NvsRational videoStreamFrameRate = aVFileInfo.getVideoStreamFrameRate(0);
        if (videoStreamFrameRate == null || videoStreamFrameRate.den <= 0 || videoStreamFrameRate.num <= 0 || aVFileInfo.getVideoStreamDuration(0) < j) {
            return false;
        }
        int detectVideoFileKeyframeInterval = instance.detectVideoFileKeyframeInterval(str);
        if (detectVideoFileKeyframeInterval < 2) {
            return false;
        }
        ...
        if (height != 0) {
            ...
            while (it.hasNext()) {
                ...
                if (floor2 > floor) {
                    ...
                    if (z) {
                        if (isOnlyDecodecKeyFrame(next.m_mediaFilePath, next.m_trimDuration / ((long) Math.max((int) Math.ceil((double) (next.m_width / this.m_thumbnailWidth)), 1)))) {
                            next.m_flags = 5;
                        } else {
                            next.m_flags = 3;
                        }
        ...
        while (it.hasNext()) {
            ...
            if (!thumbnail3.m_touched) {
                ...
                if (thumbnail3.m_imageViewUpToDate) {
                    ...
                    int i9 = (thumbnail3.m_owner.m_flags & 4) > 0 ? 1 : 0;
                    Bitmap nativeGetIconFromCache = nativeGetIconFromCache(this.m_internalObject, thumbnail3.m_owner.m_mediaFilePath, j, i9);
                    if (nativeGetIconFromCache != null) {
                        ...
                        thumbnail3.m_iconTaskId = nativeGetIcon(this.m_internalObject, thumbnail3.m_owner.m_mediaFilePath, j, i9);
                        z2 = true;
                    }
                }
            }
            ...
            if (arrayList != null) {
                ...
                while (it.hasNext()) {
                    ...
                    if (next.mediaFilePath == null || next.inPoint < j || next.outPoint <= next.inPoint || next.trimIn < 0 || next.trimOut <= next.trimIn) {
                        ...
                        if (next.onlyDecodecKeyFrame) {
                            thumbnailSequence.m_flags = 4;
                        }
